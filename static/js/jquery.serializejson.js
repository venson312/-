  ...
  $.serializeJSON = {
    ...
    setupOpts: function(options) {
      ...
      return {
        ...
        sameKeyIsReplace:optWithDefault('sameKeyIsReplace'),
        separator:optWithDefault('separator')
      };
    },
      ...
      if (keys.length === 1) {
        if (key === '') {
          o.push(value); // '' is used to push values into the array (assume o is an array)
        	...
        	if(o[key]){
        		if(opts['sameKeyIsReplace'] === false){
        			var values = o[key].split(opts['separator']);
        			values.push(value);
        			o[key] = values.join(opts['separator']);
        		}else{
        			o[key] = value;
        		}
        	}else{
